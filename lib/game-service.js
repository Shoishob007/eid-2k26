import { ensureDatabaseConnection, isDatabaseConnectionIssue, prisma } from "@/lib/prisma";
import { allowedNames, MAX_SPINS, pickFlair, pickWeightedSection, participantsSeed } from "@/lib/game-config";

const optionalNames = participantsSeed.filter((player) => player.isOptional).map((player) => player.name);
const requiredNames = participantsSeed.filter((player) => !player.isOptional).map((player) => player.name);
const participantsByName = new Map(participantsSeed.map((player) => [player.name, player]));

const DB_TEMP_UNAVAILABLE = "DB_TEMP_UNAVAILABLE";

async function withDatabaseResilience(task, retries = 1) {
    let attempt = 0;

    while (true) {
        try {
            await ensureDatabaseConnection();
            return await task();
        } catch (error) {
            if (isDatabaseConnectionIssue(error)) {
                if (attempt < retries) {
                    attempt += 1;
                    continue;
                }

                throw new Error(DB_TEMP_UNAVAILABLE);
            }

            throw error;
        }
    }
}

function toPlayerState(player) {
    const spinsLeft = Math.max(player.maxSpins - player.spinsUsed, 0);

    let status = "yet-to-spin";
    if (player.hasClaimed) {
        status = "claimed";
    } else if (player.spinsUsed === 0) {
        status = "yet-to-spin";
    } else if (spinsLeft === 0) {
        status = "exhausted";
    } else {
        status = "in-progress";
    }

    return {
        id: player.id,
        name: player.name,
        isOptional: player.isOptional,
        maxSpins: player.maxSpins,
        spinsUsed: player.spinsUsed,
        spinsLeft,
        hasClaimed: player.hasClaimed,
        claimedAmount: player.claimedAmount,
        claimedSpinOrder: player.claimedSpinOrder,
        claimedAt: player.claimedAt,
        status,
        spins: (player.spins ?? []).map((spin) => ({
            id: spin.id,
            spinOrder: spin.spinOrder,
            amount: spin.amount,
            label: spin.label,
            flair: spin.flair,
            power: spin.power,
            createdAt: spin.createdAt,
        })),
    };
}

export async function ensureParticipantsSeeded() {
    await withDatabaseResilience(async () => {
        await prisma.$transaction([
            prisma.participant.createMany({
                data: participantsSeed.map((player) => ({
                    name: player.name,
                    isOptional: player.isOptional,
                    maxSpins: MAX_SPINS,
                })),
                skipDuplicates: true,
            }),
            prisma.participant.updateMany({
                where: {
                    name: {
                        in: participantsSeed.map((player) => player.name),
                    },
                },
                data: {
                    maxSpins: MAX_SPINS,
                },
            }),
            prisma.participant.updateMany({
                where: {
                    name: {
                        in: requiredNames,
                    },
                },
                data: {
                    isOptional: false,
                },
            }),
            prisma.participant.updateMany({
                where: {
                    name: {
                        in: optionalNames,
                    },
                },
                data: {
                    isOptional: true,
                },
            }),
        ]);
    });
}

async function ensureParticipantRecord(name) {
    const participant = participantsByName.get(name);

    if (!participant) {
        throw new Error("NOT_ALLOWED");
    }

    await withDatabaseResilience(async () => {
        await prisma.participant.upsert({
            where: { name },
            create: {
                name: participant.name,
                isOptional: participant.isOptional,
                maxSpins: MAX_SPINS,
            },
            update: {
                isOptional: participant.isOptional,
                maxSpins: MAX_SPINS,
            },
        });
    });
}

export async function getDashboard(selectedName) {
    return withDatabaseResilience(async () => {
        await ensureParticipantsSeeded();

        const participants = await prisma.participant.findMany({
            include: {
                spins: {
                    orderBy: { spinOrder: "asc" },
                },
            },
            orderBy: { name: "asc" },
        });

        const players = participants.map(toPlayerState);
        const leaderboard = buildLeaderboard(players);

        const selectedPlayer = selectedName
            ? players.find((player) => player.name === selectedName) ?? null
            : null;

        return {
            players,
            leaderboard,
            selectedPlayer,
        };
    });
}

export async function spinForPlayer({ name, power }) {
    return withDatabaseResilience(async () => {
        if (!allowedNames.has(name)) {
            throw new Error("NOT_ALLOWED");
        }

        await ensureParticipantRecord(name);

        const player = await prisma.participant.findUnique({
            where: { name },
            include: {
                spins: {
                    orderBy: { spinOrder: "asc" },
                },
            },
        });

        if (!player) {
            throw new Error("NOT_FOUND");
        }

        if (player.hasClaimed) {
            throw new Error("ALREADY_CLAIMED");
        }

        if (player.spinsUsed >= player.maxSpins) {
            throw new Error("NO_SPINS_LEFT");
        }

        const section = pickWeightedSection();
        const flair = pickFlair();
        const spinOrder = player.spinsUsed + 1;

        await prisma.$transaction([
            prisma.spin.create({
                data: {
                    participantId: player.id,
                    spinOrder,
                    amount: section.amount,
                    label: section.label,
                    flair,
                    power,
                },
            }),
            prisma.participant.update({
                where: { id: player.id },
                data: {
                    spinsUsed: {
                        increment: 1,
                    },
                },
            }),
        ]);

        const fresh = await prisma.participant.findUnique({
            where: { id: player.id },
            include: {
                spins: {
                    orderBy: { spinOrder: "asc" },
                },
            },
        });

        return toPlayerState(fresh);
    }, 0);
}

export async function claimSpin({ name, spinOrder }) {
    return withDatabaseResilience(async () => {
        if (!allowedNames.has(name)) {
            throw new Error("NOT_ALLOWED");
        }

        await ensureParticipantRecord(name);

        const player = await prisma.participant.findUnique({
            where: { name },
            include: {
                spins: {
                    orderBy: { spinOrder: "asc" },
                },
            },
        });

        if (!player) {
            throw new Error("NOT_FOUND");
        }

        if (player.hasClaimed) {
            throw new Error("ALREADY_CLAIMED");
        }

        const selectedSpin = player.spins.find((spin) => spin.spinOrder === spinOrder);

        if (!selectedSpin) {
            throw new Error("SPIN_NOT_FOUND");
        }

        await prisma.participant.update({
            where: { id: player.id },
            data: {
                hasClaimed: true,
                claimedAmount: selectedSpin.amount,
                claimedSpinOrder: selectedSpin.spinOrder,
                claimedSpinId: selectedSpin.id,
                claimedAt: new Date(),
            },
        });

        const fresh = await prisma.participant.findUnique({
            where: { id: player.id },
            include: {
                spins: {
                    orderBy: { spinOrder: "asc" },
                },
            },
        });

        return toPlayerState(fresh);
    }, 0);
}

function buildLeaderboard(players) {
    const sorted = [...players].sort((a, b) => {
        const aAmount = a.claimedAmount ?? -1;
        const bAmount = b.claimedAmount ?? -1;

        if (aAmount !== bAmount) {
            return bAmount - aAmount;
        }

        if (a.spinsUsed !== b.spinsUsed) {
            return b.spinsUsed - a.spinsUsed;
        }

        return a.name.localeCompare(b.name);
    });

    return sorted.map((player, index) => ({
        rank: index + 1,
        name: player.name,
        status: player.status,
        claimedAmount: player.claimedAmount,
        spinsUsed: player.spinsUsed,
        spinsLeft: player.spinsLeft,
        hasClaimed: player.hasClaimed,
        isOptional: player.isOptional,
    }));
}
